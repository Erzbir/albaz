...
import com.erzbir.albaz.api.DefaultDispatcher;
    ...
    private Runnable createInvokeRunnable(E event, Listener<?> listener) {
        return () -> {
            try {
                ...
                ListenerResult listenerResult = DefaultDispatcher.INSTANCE.getConfiguration().getInvoker().invoke(new DefaultListenerContext(new DefaultEventContext(event), listener));
