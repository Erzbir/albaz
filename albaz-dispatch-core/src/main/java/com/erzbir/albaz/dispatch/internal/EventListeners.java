...
public final class EventListeners {
    ...
    public EventListeners() {
        listeners.put(Listener.Priority.HIGH, new ArrayList<>());
        listeners.put(Listener.Priority.LOW, new ArrayList<>());
        listeners.put(Listener.Priority.NORMAL, new ArrayList<>());
    }

    public void addListener(ListenerRegistry registry, Listener.Priority priority) {
        listeners.get(priority).add(registry);
    }

    public void addListener(ListenerRegistry registry) {
        listeners.get(Listener.Priority.HIGH).add(registry);
    }


    public void removeListener(Listener<?> listener, Listener.Priority priority) {
        listeners.get(priority).removeIf(registry -> registry.listener().equals(listener));
    }

    public void removeListener(Listener<?> listener) {
        Function<ListenerRegistry, Boolean> function = registry -> registry.listener().equals(listener);
        if (listeners.get(Listener.Priority.HIGH).removeIf(function::apply)) {
            return;
        } else if (listeners.get(Listener.Priority.LOW).removeIf(function::apply)) {
            return;
        }
        listeners.get(Listener.Priority.NORMAL).removeIf(function::apply);
    }
